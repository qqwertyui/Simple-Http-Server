...
Socket::Socket(std::string interface, const int port, const int timeout) {
  ...
  if(setsockopt(this->fd, SOL_SOCKET, SO_RCVTIMEO, (char *)&to, sizeof(to)) == -1) {
    throw std::runtime_error(strerror(errno));
  }
  int reuseaddr = 1;
  if(setsockopt(this->fd, SOL_SOCKET, SO_REUSEADDR, &reuseaddr, sizeof(reuseaddr)) == -1) {
    throw std::runtime_error(strerror(errno));
  }
  ...
  if(bind(this->fd, (const sockaddr*)&this->socket_info, sizeof(sockaddr)) == -1) {
    throw std::runtime_error(strerror(errno));
  }
}
...
Peer_info::Peer_info(uint32_t fd, uint16_t port, std::string ip) {
  this->fd = fd;
  this->port = port;
  this->ip = ip;
...
uint32_t Peer_info::get_fd() {
  return this->fd;
}

uint16_t Peer_info::get_port() {
  return this->port;
}

std::string Peer_info::get_ip() {
  return this->ip;
}